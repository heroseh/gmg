...
static inline GmgResult _gmg_logical_device_object_get(GmgLogicalDevice* logical_device, _GmgLogicalDeviceObjectId id, _GmgLogicalDeviceObject** out) {
	if (id.raw == 0) return GmgResult_error_unexpected_null_id;
	if (!DasPool_is_id_valid(_GmgLogicalDeviceObjectId, &logical_device->object_pool, id))
		return GmgResult_error_object_use_after_free;
...
static GmgResult _gmg_vulkan_device_memory_block_init(GmgLogicalDevice* logical_device, _GmgVulkanBlock* block, GmgMemoryLocation memory_location, uint8_t memory_type_idx, uint64_t size) {
	...
	if (memory_location != GmgMemoryLocation_gpu) {
	...
	if (free_range == NULL) return GmgResult_error_out_of_memory_host;
	free_range->start = 0;
	free_range->end = size;
...
static GmgResult _gmg_vulkan_device_memory_block_deinit(GmgLogicalDevice* logical_device, GmgMemoryLocation memory_location, _GmgVulkanBlock* block) {
...
static GmgResult _gmg_vulkan_device_memory_block_alloc(GmgLogicalDevice* logical_device, _GmgVulkanBlock* block, _GmgVulkanAllocArgs* args, _GmgVulkanAllocResult* out) {
	...
	if (smallest_found_range_idx != UINT32_MAX) {
		...
		if (allocation == NULL) return GmgResult_error_out_of_memory_host;
		...
		out->memory_mapped_data = das_ptr_add(block->mapped_memory, allocation_start);
	...
	return GmgResult_error_out_of_memory_device;
}

GmgResult _gmg_vulkan_device_memory_block_dealloc(_GmgVulkanBlock* block, uint64_t start, uint64_t end) {
	//
...
static GmgResult _gmg_vulkan_device_memory_pool_deinit(GmgLogicalDevice* logical_device, _GmgVulkanPool* pool) {
	for (uint32_t i = 0; i < pool->blocks_count; i += 1) {
		...
		if (key != 0) {
			GmgMemoryLocation memory_location = (key & _GmgVulkanBlockKey_memory_location_MASK) >> _GmgVulkanBlockKey_memory_location_SHIFT;
...
static GmgResult _gmg_vulkan_device_memory_pool_alloc(GmgLogicalDevice* logical_device, _GmgVulkanPool* pool, _GmgVulkanAllocArgs* args, _GmgVulkanAllocResult* out) {
	...
	GmgMemoryLocation memory_location = *args->memory_location;
	for (uint32_t i = 0; i < pool->blocks_count; i += 1) {
		_GmgVulkanBlockKey key = pool->keys[i];
		if (key == 0) {
		...
		if (result == GmgResult_error_out_of_memory_device) {
			continue;
		}
		if (result < 0) return result;
		goto SUCCESS;
	...
	uint64_t block_size = memory_location == GmgMemoryLocation_gpu
	...
	das_assert(result != GmgResult_error_out_of_memory_device, "a new memory block in a pool must be able to allocate the initial allocation");
	if (result < 0) return result;
...
static GmgResult _gmg_vulkan_device_memory_pool_dealloc(GmgLogicalDevice* logical_device, _GmgVulkanPool* pool, uint32_t block_idx, uint64_t start, uint64_t end) {
	...
	GmgResult result = _gmg_vulkan_device_memory_block_dealloc(block, start, end);
	if (result < 0) return result;
...
MEMORY_TYPE_FOUND: {}
	if (memory_properties & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) {
		if (memory_properties & VK_MEMORY_PROPERTY_HOST_CACHED_BIT) {
			*args->memory_location = GmgMemoryLocation_shared_cached;
		} else {
			*args->memory_location = GmgMemoryLocation_shared;
		}
	}
...
GmgResult _gmg_vulkan_device_memory_allocator_flush(GmgLogicalDevice* logical_device, _GmgVulkanAllocId alloc_id, void* memory_mapped_start, uint64_t offset, uintptr_t size) {
	...
	if (range == NULL) return GmgResult_error_out_of_memory_host;
...
static GmgResult _gmg_vulkan_memory_stager_stage(GmgLogicalDevice* logical_device, uint32_t size, uint32_t align, _GmgVulkanStagingBuffer** staging_buffer_out, uint32_t* src_buffer_offset_out, void** src_buffer_data_out) {
	...
	if (buffer == NULL) {
		...
		if (buffer == NULL) return GmgResult_error_out_of_memory_host;
...

		{
			...
			VkResult vk_result = vkCreateBuffer(logical_device->vulkan.handle, &vk_create_info, GMG_TODO_VULKAN_ALLOCATOR, &buffer->buffer);
			if (vk_result) return _gmg_vulkan_convert_from_result(vk_result);


			VkMemoryRequirements mem_req;
			vkGetBufferMemoryRequirements(logical_device->vulkan.handle, buffer->buffer, &mem_req);

			GmgMemoryLocation memory_location = GmgMemoryLocation_shared;
			...
			buffer->memory_mapped_data = alloc_result.memory_mapped_data;
			buffer->device_memory_alloc_id = alloc_result.id;

			vk_result = vkBindBufferMemory(logical_device->vulkan.handle, buffer->buffer, alloc_result.device_memory, alloc_result.device_memory_offset);
			if (vk_result) return _gmg_vulkan_convert_from_result(vk_result);
		}
	}
	...
	*src_buffer_data_out = das_ptr_add(buffer->memory_mapped_data, aligned_pos);
...
static GmgResult _gmg_vulkan_convert_from_result(VkResult result) {
	switch (result) {
		...
		case VK_ERROR_OUT_OF_HOST_MEMORY: return GmgResult_error_out_of_memory_host;
		case VK_ERROR_OUT_OF_DEVICE_MEMORY: return GmgResult_error_out_of_memory_device;
		default:
...
static GmgResult _gmg_vulkan_convert_to_shader_stage_flags(GmgShaderStageFlags gmg, VkShaderStageFlags* vk_out) {
	...
	if (gmg & GmgShaderStageFlags_all_graphics) vk |= VK_SHADER_STAGE_ALL_GRAPHICS;
...
static GmgResult _gmg_vulkan_convert_to_render_pass_args(GmgLogicalDevice* logical_device,
	...
	if (vk_attachments == NULL) return GmgResult_error_out_of_memory_host;
	...
	for (uint32_t i = 0; i < attachments_count; i += 1) {
		...
		if (is_depth) {
			...
			if (vk_depth_ref) return GmgResult_error_multiple_depth_stencil_attachments;
			vk_depth_ref = ref;
		} else {
			ref = DasStk_push(&vk_color_refs, NULL);
		}
		if (ref == NULL) return GmgResult_error_out_of_memory_host;
		ref->attachment = i;
...
static int _gmg_vulkan_command_buffer_build_transfer_job(void* args_) {
	...
	for (uint32_t i = 0; i < stager->filled_buffers_count + 1; i += 1) {
		...
		VkBuffer src_buffer = buffer->buffer;
...
static int _gmg_vulkan_command_buffer_build_graphics_job(void* args_) {
	...
	VkClearValue* vk_clear_values = das_alloc_array(VkClearValue, temp_alctor, frame_buffer->attachments_count);
	if (vk_clear_values == NULL) {
		result = GmgResult_error_out_of_memory_host;
		goto END;
	}
	...
	VkRenderPassBeginInfo render_pass_being_info = {
		...
		.clearValueCount = frame_buffer->attachments_count,
...
static GmgResult _gmg_vulkan_backend_init(GmgSetup* setup) {
	...
	//
	{
		...
		if (physical_devices == NULL) return GmgResult_error_out_of_memory_host;
		...
		if (_gmg.physical_devices == NULL) return GmgResult_error_out_of_memory_host;
...
static GmgResult _gmg_vulkan_backend_physical_device_surface_texture_format(GmgPhysicalDevice* physical_device, GmgSurface surface, GmgTextureFormat* out) {
	...
	if (physical_device->vulkan.surface_formats == NULL) {
		...
		if (physical_device->vulkan.surface_formats == NULL) return GmgResult_error_out_of_memory_host;
...
static GmgResult _gmg_vulkan_backend_logical_device_init(GmgPhysicalDevice* physical_device, GmgLogicalDevice* logical_device, GmgLogicalDeviceCreateArgs* create_args) {
	...
	for (_GmgVulkanQueueType queue_type = 0; queue_type < _GmgVulkanQueueType_primary_end; queue_type += 1) {
		if (queue_type_to_score[queue_type] == 0) {
			return GmgResult_error_cannot_find_graphics_queue + queue_type;
		}
	}
...
static GmgResult _gmg_vulkan_backend_logical_device_submit(GmgLogicalDevice* logical_device, GmgRenderPassId* ordered_render_pass_ids, uint32_t render_passes_count) {
	...
	//
	// write to the descriptors that have been updated since the last submit
	//
	uint32_t descriptor_writes_count = DasStk_count(&logical_device->set_descriptors_queue);
	if (descriptor_writes_count) {
		...
		DasStk_foreach(&logical_device->set_descriptors_queue, i) {
		...
		DasStk_clear(&logical_device->set_descriptors_queue);
	}
	...
	if (DasStk_count(&stager->buffers)) {
		for (uint32_t i = 0; i < stager->filled_buffers_count + 1; i += 1) {
			...
			if (buffer->needs_flush) {
				result = _gmg_vulkan_device_memory_allocator_flush(logical_device, buffer->device_memory_alloc_id, buffer->memory_mapped_data, 0, buffer->position);
				if (result < 0) return result;
			}
		}
	...
	if (flushes_count) {
		...
		if (vk_ranges == NULL) return GmgResult_error_out_of_memory_host;
	...
	//
	// recycle any vulkan resources (command buffers and fences) from previous
	...
	while (DasDeque_count(&logical_device->vulkan.graphics_submits)) {
		...
		if (vk_result == VK_SUCCESS) {
			VkCommandBuffer* insert_ptr = DasStk_push_many(&logical_device->vulkan.graphics_command_buffers_available, NULL, submit->command_buffers_count);
			if (insert_ptr == NULL) return GmgResult_error_out_of_memory_host;

			DasDeque_read(&logical_device->vulkan.graphics_command_buffers_being_executed, 0, insert_ptr, submit->command_buffers_count);
			DasDeque_pop_front_many(&logical_device->vulkan.graphics_command_buffers_being_executed, submit->command_buffers_count);

			vk_result = vkResetFences(logical_device->vulkan.handle, 1, &submit->fence);
			if (vk_result) return _gmg_vulkan_convert_from_result(vk_result);

			VkFence* fence = DasStk_push(&logical_device->vulkan.graphics_fences_available, NULL);
			if (fence == NULL) return GmgResult_error_out_of_memory_host;
			*fence = submit->fence;
	...
	//
	_GmgVulkanCommandBufferBuildGraphics* graphics_builds = das_alloc_array(_GmgVulkanCommandBufferBuildGraphics, temp_alctor, render_passes_count);
	if (graphics_builds == NULL) return GmgResult_error_out_of_memory_host;

	for (uint32_t i = 0; i < render_passes_count; i += 1) {
		result = gmg_render_pass_get(logical_device, ordered_render_pass_ids[i], &graphics_builds[i].render_pass);
		if (result < 0) return result;
		...
		callback_args.execute_job.args = &graphics_builds[i];
		_gmg.callback_impl_fn(logical_device, GmgCallbackFn_execute_job, &callback_args, _gmg.callback_userdata);
	}
	...
	VkFence fence;
	if (DasStk_count(&logical_device->vulkan.graphics_fences_available)) {
		fence = *DasStk_get_last(&logical_device->vulkan.graphics_fences_available);
		DasStk_pop(&logical_device->vulkan.graphics_fences_available);
	} else {
		...
		VkResult vk_result = vkCreateFence(logical_device->vulkan.handle, &fence_create_info, GMG_TODO_VULKAN_ALLOCATOR, &fence);
		if (vk_result) return _gmg_vulkan_convert_from_result(vk_result);
	}

	vk_result = vkQueueSubmit(logical_device->vulkan.queue_type_to_queue[_GmgVulkanQueueType_graphics], 1, &vk_submit_info, fence);
	if (vk_result) return _gmg_vulkan_convert_from_result(vk_result);

	uintptr_t insert_idx = DasDeque_push_back_many(&logical_device->vulkan.graphics_command_buffers_being_executed, NULL, command_buffers_count);
	if (insert_idx == UINTPTR_MAX) {
		return GmgResult_error_out_of_memory_host;
	}


	DasDeque_write(&logical_device->vulkan.graphics_command_buffers_being_executed, insert_idx, queued_command_buffers, command_buffers_count);
	...
	submit->fence = fence;
	submit->command_buffers_count = command_buffers_count;
...
static GmgResult _gmg_vulkan_backend_texture_reinit(GmgTexture* texture, GmgLogicalDevice* logical_device) {
	...
	GmgMemoryLocation memory_location = GmgMemoryLocation_gpu;
	_GmgVulkanAllocResult alloc_result;
	_GmgVulkanAllocArgs args = {
		.size = mem_req.size,
		.align = mem_req.alignment,
	...
	texture->vulkan.memory_mapped_data = alloc_result.memory_mapped_data;
...
VK_PIPELINE_INIT_SHADER_END: {}
	...
	VkPipelineVertexInputStateCreateInfo vk_vertex_input;
	{
		...
		if (vk_binding_descs == NULL) return GmgResult_error_out_of_memory_host;
		...
		if (vk_attribs == NULL) return GmgResult_error_out_of_memory_host;
		...
		for (uint32_t i = 0; i < vertex_layout->bindings_count; i += 1) {
			...
			for (uint32_t j = 0; j < binding->attribs_count; j += 1) {
				...
				if (vk_attrib == NULL) return GmgResult_error_out_of_memory_host;
...
static GmgResult _gmg_vulkan_backend_buffer_reinit(GmgBuffer* buffer, GmgLogicalDevice* logical_device, uint64_t old_size, uint64_t new_size) {
	...
	_GmgVulkanAllocArgs args = {
		...
		.align = mem_req.alignment,
		.memory_type_bits = mem_req.memoryTypeBits,
		.memory_location = &buffer->memory_location,
		.type = _GmgVulkanAllocType_buffer,
	};
	result = _gmg_vulkan_device_memory_alloc(logical_device, &args, &alloc_result, &buffer->vulkan.needs_flush);
	if (result < 0) return result;

	buffer->vulkan.memory_mapped_data = alloc_result.memory_mapped_data;
...
static GmgResult _gmg_vulkan_backend_buffer_get_read_buffer(GmgBuffer* buffer, GmgLogicalDevice* logical_device, uint64_t offset, const void** data_out) {
	GmgResult result;
	VkResult vk_result;
	*data_out = das_ptr_add(buffer->vulkan.memory_mapped_data, offset);
	return GmgResult_success;
}
static GmgResult _gmg_vulkan_backend_buffer_get_write_buffer(GmgBuffer* buffer, GmgLogicalDevice* logical_device, uint64_t offset, uintptr_t size, void** data_out) {
	GmgResult result;
	VkResult vk_result;
	if (buffer->memory_location == GmgMemoryLocation_gpu) {
		result = _gmg_vulkan_memory_stager_stage_buffer(logical_device, buffer->vulkan.handle, offset, size, data_out);
		if (result < 0) return result;
	} else {
		*data_out = das_ptr_add(buffer->vulkan.memory_mapped_data, offset);
		if (buffer->vulkan.needs_flush) {
			result = _gmg_vulkan_device_memory_allocator_flush(logical_device, buffer->vulkan.device_memory_alloc_id, buffer->vulkan.memory_mapped_data, offset, size);
			if (result < 0) return result;
		}
	}
...
static GmgResult _gmg_vulkan_backend_swapchain_reinit(GmgSwapchain* swapchain, GmgLogicalDevice* logical_device, GmgSwapchainCreateArgs* create_args) {
	...
	// get the prensent queue
	{
		...
		if (swapchain->vulkan.present_queue == VK_NULL_HANDLE) {
			return GmgResult_error_cannot_find_present_queue;
		}
	}
	...
	if (images == NULL) return GmgResult_error_out_of_memory_host;
	...
	if (swapchain->texture_ids == NULL) return GmgResult_error_out_of_memory_host;
...
static  const char* _GmgResult_strings[GmgResult_COUNT] = {
	[GmgResult_error_ - GmgResult_error_END] = "",
	[GmgResult_error_unexpected_null_id - GmgResult_error_END] = "unexpected_null_id",
	[GmgResult_error_object_use_after_free - GmgResult_error_END] = "object_use_after_free",
	[GmgResult_error_logical_device_object_pool_full - GmgResult_error_END] = "logical_device_object_pool_full",
	[GmgResult_error_out_of_memory_host - GmgResult_error_END] = "out_of_memory_host",
	[GmgResult_error_out_of_memory_device - GmgResult_error_END] = "out_of_memory_device",
	[GmgResult_error_multiple_depth_stencil_attachments - GmgResult_error_END] = "multiple_depth_stencil_attachments",
	[GmgResult_error_cannot_find_transfer_queue - GmgResult_error_END] = "cannot_find_transfer_queue",
	[GmgResult_error_cannot_find_compute_queue - GmgResult_error_END] = "cannot_find_compute_queue",
	[GmgResult_error_cannot_find_graphics_queue - GmgResult_error_END] = "cannot_find_graphics_queue",
	[GmgResult_error_cannot_find_present_queue - GmgResult_error_END] = "cannot_find_present_queue",
	[GmgResult_error_incompatible_shader_format - GmgResult_error_END] = "incompatible_shader_format",
	[GmgResult_error_vertex_layout_pool_full - GmgResult_error_END] = "vertex_layout_pool_full",
	[GmgResult_error_layout_size_mismatch - GmgResult_error_END] = "layout_size_mismatch",
	[GmgResult_error_incompatible_backend_type - GmgResult_error_END] = "incompatible_backend_type",
	[GmgResult_error_missing_vertex_shader - GmgResult_error_END] = "missing_vertex_shader",
	[GmgResult_error_missing_fragment_shader - GmgResult_error_END] = "missing_fragment_shader",
	[GmgResult_error_missing_mesh_shader - GmgResult_error_END] = "missing_mesh_shader",
	[GmgResult_error_missing_compute_shader - GmgResult_error_END] = "missing_compute_shader",
	[GmgResult_error_binding_idx_out_of_bounds - GmgResult_error_END] = "binding_idx_out_of_bounds",
	[GmgResult_error_descriptor_type_mismatch - GmgResult_error_END] = "descriptor_type_mismatch",
	[GmgResult_error_binding_is_not_dynamic - GmgResult_error_END] = "binding_is_not_dynamic",
	[GmgResult_error_binding_elmt_idx_out_of_bounds - GmgResult_error_END] = "binding_elmt_idx_out_of_bounds",
	[GmgResult_error_expected_vertex_buffer - GmgResult_error_END] = "expected_vertex_buffer",
	[GmgResult_error_incompatible_binding_idx - GmgResult_error_END] = "incompatible_binding_idx",
	[GmgResult_error_expected_index_buffer - GmgResult_error_END] = "expected_index_buffer",
	[GmgResult_error_incompatible_render_pass_layout - GmgResult_error_END] = "incompatible_render_pass_layout",
	[GmgResult_error_incompatible_vertex_layout - GmgResult_error_END] = "incompatible_vertex_layout",
	[GmgResult_error_draw_cmd_already_in_process - GmgResult_error_END] = "draw_cmd_already_in_process",
	[GmgResult_error_uninitialized - GmgResult_error_END] = "uninitialized",
	[GmgResult_error_push_constants_out_of_bounds - GmgResult_error_END] = "push_constants_out_of_bounds",
	[GmgResult_error_vertex_array_has_index_buffer - GmgResult_error_END] = "vertex_array_has_index_buffer",
	[GmgResult_error_vertex_array_does_not_have_index_buffer - GmgResult_error_END] = "vertex_array_does_not_have_index_buffer",
	[GmgResult_error_buffer_start_idx_out_of_bounds - GmgResult_error_END] = "buffer_start_idx_out_of_bounds",
	[GmgResult_error_cannot_read_from_gpu_only_resource - GmgResult_error_END] = "cannot_read_from_gpu_only_resource",
	[GmgResult_error_map_out_of_bounds - GmgResult_error_END] = "map_out_of_bounds",
	[GmgResult_error_attachment_index_out_of_bounds - GmgResult_error_END] = "attachment_index_out_of_bounds",
	[GmgResult_error_width_must_not_be_zero - GmgResult_error_END] = "width_must_not_be_zero",
	[GmgResult_error_height_must_not_be_zero - GmgResult_error_END] = "height_must_not_be_zero",
...
const char* GmgResult_string(GmgResult result) {
	return _GmgResult_strings[result - GmgResult_error_END];
}
...
GmgResult gmg_vertex_layout_register(GmgVertexLayout* vl, GmgVertexLayoutId* id_out) {
	...
	if (dst_vl == NULL) return GmgResult_error_vertex_layout_pool_full;

	GmgVertexBindingInfo* bindings = das_alloc_array(GmgVertexBindingInfo, GMG_TODO_INTERNAL_ALLOCATOR, vl->bindings_count);
	if (bindings == NULL) return GmgResult_error_out_of_memory_host;
	for (uint32_t i = 0; i < vl->bindings_count; i += 1) {
		...
		if (attribs == NULL) return GmgResult_error_out_of_memory_host;
		...
		if (size != src->size) {
			return GmgResult_error_layout_size_mismatch;
		}
...
GmgResult gmg_logical_device_init(uint32_t physical_device_idx, GmgLogicalDeviceCreateArgs* create_args, GmgLogicalDevice** out) {
	...
	if (logical_device == NULL) return GmgResult_error_out_of_memory_host;
...
GmgResult gmg_texture_resize(GmgLogicalDevice* logical_device, GmgTextureId id, uint32_t width, uint32_t height, uint32_t depth, uint32_t mip_levels, uint32_t array_layers_count) {
	if (width == 0) {
		return GmgResult_error_width_must_not_be_zero;
	}
	if (height == 0) {
		return GmgResult_error_height_must_not_be_zero;
	}
	if (depth == 0) {
		return GmgResult_error_depth_must_not_be_zero;
	}
	if (mip_levels == 0) {
		return GmgResult_error_mip_levels_must_not_be_zero;
	}
	if (array_layers_count == 0) {
		return GmgResult_error_array_layers_must_not_be_zero;
	}
	...
	switch (create_args->type) {
		...
		case GmgShaderType_compute:
			if (create_args->stages.compute.format == GmgShaderFormat_none) {
				return GmgResult_error_missing_compute_shader;
			}
			break;
	}
...
GmgResult gmg_buffer_get_read_buffer(GmgLogicalDevice* logical_device, GmgBufferId id, uint64_t start_idx, uintptr_t elmts_count, const void** data_out) {
	...
	if (buffer->memory_location == GmgMemoryLocation_gpu) {
		return GmgResult_error_cannot_read_from_gpu_only_resource;
	}

	if (start_idx + elmts_count > buffer->elmts_count) {
		return GmgResult_error_map_out_of_bounds;
	}

	uint64_t offset = start_idx * (uint64_t)buffer->elmt_size;
	return _gmg_backend_fn_call(buffer_get_read_buffer, buffer, logical_device, offset, data_out);
}
...
GmgResult gmg_vertex_array_init(GmgLogicalDevice* logical_device, GmgVertexArrayCreateArgs* create_args, GmgVertexArrayId* id_out) {
	...
	if (vertex_buffer_ids == NULL) return GmgResult_error_out_of_memory_host;
	...
	for (uint32_t i = 0; i < create_args->vertex_buffers_count; i += 1) {
		...
		if (buffer->type != GmgBufferType_vertex) {
			return GmgResult_error_expected_vertex_buffer;
		}
		if (buffer->vertex.binding_idx != i) {
			return GmgResult_error_incompatible_binding_idx;
		}
	}
	...
	if (create_args->index_buffer_id.raw) {
		...
		if (buffer->type != GmgBufferType_index) {
			return GmgResult_error_expected_index_buffer;
		}
	}
...
GmgResult gmg_render_pass_layout_init(GmgLogicalDevice* logical_device, GmgRenderPassLayoutCreateArgs* create_args, GmgRenderPassLayoutId* id_out) {
	...
	if (attachments == NULL) return GmgResult_error_out_of_memory_host;
...
GmgResult gmg_render_pass_set_frame_buffer(GmgLogicalDevice* logical_device, GmgRenderPassId id, GmgFrameBufferId frame_buffer_id) {
	...
	if (frame_buffer->render_pass_layout_id.raw != render_pass->layout_id.raw) {
		return GmgResult_error_incompatible_render_pass_layout;
	}
...
GmgResult gmg_render_pass_set_attachment_clear_value(GmgLogicalDevice* logical_device, GmgRenderPassId id, uint16_t attachment_idx, GmgClearValue* clear_value) {
	...
	if (attachment_idx >= render_pass->attachments_count)
		return GmgResult_error_attachment_index_out_of_bounds;
...
GmgResult gmg_render_pass_draw_cmd_start(GmgLogicalDevice* logical_device, GmgRenderPassId id, GmgMaterialId material_id, GmgVertexArrayId vertex_array_id, GmgDrawCmdBuilder* builder_in_out) {
	...
	if (vertex_array->layout_id.raw != material_spec->vertex_layout_id.raw) {
		return GmgResult_error_incompatible_vertex_layout;
	}
...
GmgResult gmg_render_pass_draw_cmd_set_push_constants(GmgDrawCmdBuilder* builder, void* data, uint32_t offset, uint32_t size) {
	...
	if (offset + size > shader->push_constants_size) {
		return GmgResult_error_push_constants_out_of_bounds;
	}

	builder->draw_cmd.push_constants_data = data;
...
GmgResult gmg_render_pass_draw_cmd_queue_vertexed(GmgDrawCmdBuilder* builder, uint32_t vertices_start_idx, uint32_t vertices_count) {
	...
	if (vertex_array->index_buffer_id.raw != 0) {
		return GmgResult_error_vertex_array_has_index_buffer;
	}
...
GmgResult gmg_render_pass_draw_cmd_queue_indexed(GmgDrawCmdBuilder* builder, uint32_t vertices_start_idx, uint32_t indices_start_idx, uint32_t indices_count) {
	...
	if (vertex_array->index_buffer_id.raw == 0) {
		return GmgResult_error_vertex_array_does_not_have_index_buffer;
	}
...
GmgResult _gmg_vulkan_instance_layers_get(VkLayerProperties** properties_out, uint32_t* properties_count_out) {
	...
	if (properties == NULL) return GmgResult_error_out_of_memory_host;
